...
# frist things first, let's clear out the contents of the table
table.clear()
...
for dictionary_key , dictionary_value in inventory.items():
	...
	# add a blank colum for next set of data points
	table.appendCol()

	# add ot the list so we can determine the max length of the table
	table_length.append( len( dictionary_value ) )

# set the table length based on the maxium number of dictionary entries
